...
import { createEditorTools } from "../../src/main/story/integration/tools/editor/editorTools.ts";
import type { RendererEditorToolClient } from "../../src/main/story/integration/tools/editor/contracts.ts";
import type { RegisteredTool } from "../../src/main/story/integration/StoryToolResolver.ts";
import { createToolApprovalPreview } from "../../src/main/story/integration/StoryToolPreview.ts";
...
describe("live editor tools", () => {
  it("reads live context and applies revision-checked text edits", async () => {
    ...
    expect(client.invoke).toHaveBeenNthCalledWith(1, "project-1", { kind: "get_context" });
  ...
  it("allows revision-checked editor operations under the current desktop policy", () => {
    const policy = new ToolPolicy();

    expect(policy.getPermission("get_active_editor_context")).toBe("allow");
    expect(policy.getPermission("replace_active_editor_range")).toBe("allow");
    expect(policy.getPermission("format_active_editor_selection")).toBe("allow");
    expect(policy.getPermission("style_active_editor_selection")).toBe("allow");
    ...
    expect(policy.getPermission("apply_active_editor_styles")).toBe("allow");
  });
