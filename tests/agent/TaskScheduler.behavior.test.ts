...
import type { AgentRunResult } from "../../src/main/agent/runtime/AgentRuntime.ts";
import RunBudget, { type RunLimits } from "../../src/main/agent/runtime/RunLimits.ts";
import type {
  OrchestrationEvent,
  PlannedExecutionPlan,
  ReviewResult,
} from "../../src/main/agent/orchestration/contracts.ts";
import type {
...
} from "../../src/main/agent/orchestration/ports.ts";
import TaskScheduler from "../../src/main/agent/orchestration/TaskScheduler.ts";
...
function completedResult(taskId: string, agentType: string, attempt: number): AgentRunResult {
  return {
    status: "completed",
...
describe("TaskScheduler behavior", () => {
  it("runs dependencies in order and synthesizes only approved results", async () => {
    ...
    const runner: PlannedTaskRunner = {
      runTask: vi.fn(async (request: TaskExecutionRequest) => {
        ...
        if (request.task.id === "review") {
          expect(request.dependencyResults.map((result) => result.taskId)).toEqual(["analyze"]);
        }
        return completedResult(request.task.id, request.task.assignedAgentId, request.attempt);
      }),
    };
    ...
    const synthesizer: AnswerSynthesisProvider = {
      synthesize: vi.fn(async (request: SynthesisRequest) => {
        expect(request.results.map((result) => result.taskId)).toEqual(["analyze", "review"]);
        return "final answer";
      }),
    };
    ...
    await expect(scheduler.run(createScheduleRequest(events))).resolves.toBe("final answer");
  ...
  it("retries a task when review requests a retry", async () => {
    ...
    const runner: PlannedTaskRunner = {
      runTask: vi.fn(async (request: TaskExecutionRequest) =>
        completedResult(request.task.id, request.task.assignedAgentId, request.attempt),
      ),
    };
    const reviewer: ResultReviewProvider = {
      review: vi.fn(
        async (request: ReviewRequest): Promise<ReviewResult> =>
