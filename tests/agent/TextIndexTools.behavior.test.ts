...
import WorkspaceToolContext from "../../src/main/story/integration/StoryWorkspaceToolContext.ts";
...
import SqliteTextIndexStore from "../../src/main/story/storage/project/SqliteTextIndexStore.ts";
...
describe("local text index tools", () => {
  it("persists a structural index and refreshes changed files", async () => {
    ...
    expect(database.handle.prepare("SELECT count(*) AS count FROM indexed_files").get()).toEqual({
      count: 1,
    });
    expect((await index.getChunks()).some((chunk) => chunk.path.includes(".storyos"))).toBe(false);
    ...
    expect((await index.search("lighthouse route"))[0]?.chunk.path).toBe("chapter-one.md");
  });
  ...
  it("searches Chinese text with FTS5 and removes deleted files", async () => {
    ...
    writeFileSync(chapterPath, "# 第十章\n\n林夏最终选择沿着灯塔路线前进。", "utf8");
    const index = new TextIndexService(context);

    expect((await index.search("灯塔路线"))[0]?.chunk.path).toBe("chapter-cn.md");
    rmSync(chapterPath);
    expect(await index.search("灯塔路线")).toHaveLength(0);
    expect(database.handle.prepare("SELECT count(*) AS count FROM text_chunks").get()).toEqual({
      count: 0,
    });
  });
  ...
  it("assembles relevant context within the requested token budget", async () => {
    ...
    expect(response.result.estimated_tokens).toBeLessThanOrEqual(response.result.token_budget);
    expect(response.result.assembled_context).toContain('<source path="notes.md"');
  });
