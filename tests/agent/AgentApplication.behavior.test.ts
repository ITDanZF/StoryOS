...
import { RunTimedOutError } from "../../src/main/agent/runtime/RunLimits.ts";
...
describe("AgentApplication behavior", () => {
  ...
  it("pauses for tool approval and resumes with the selected decision", async () => {
    ...
    const runId = application.startRun(runRequest("thread-approval", "write the story"));
    const requested = await waitForEvent(events, (event) => event.type === "approval.requested");
    ...
    await expect(application.waitForRun(runId)).resolves.toBe("decision:allow_once");
  ...
  it("classifies a user cancellation as an aborted run", async () => {
    ...
    const runId = application.startRun(runRequest("thread-cancel", "keep working"));
    expect(application.cancelRun(runId)).toBe(true);

    await expect(application.waitForRun(runId)).rejects.toThrow(`Run cancelled by user: ${runId}`);
    expect(application.getRun(runId)).toMatchObject({
      runId,
      status: "aborted",
    });
  ...
  it("classifies runner timeouts separately from ordinary failures", async () => {
    ...
    const runId = application.startRun(runRequest("thread-timeout", "slow task"));

    await expect(application.waitForRun(runId)).rejects.toThrow("Agent run timed out after 250ms.");
    expect(application.getRun(runId)).toMatchObject({
      runId,
      status: "timed_out",
    });
  ...
  it("keeps private subagent lifecycle events out of the conversation stream", async () => {
    ...
    const runId = application.startRun(runRequest("thread-lineage", "delegate"));
  ...
  it("publishes main-agent tool lifecycle events with a stable call id", async () => {
    ...
    const runId = application.startRun(runRequest("thread-main-tool", "create a chapter"));
    await expect(application.waitForRun(runId)).resolves.toBe("done");
