...
} from "../../src/main/agent/runtime/AgentRuntime.ts";
import RunBudget from "../../src/main/agent/runtime/RunLimits.ts";
...
describe("AgentRuntime behavior", () => {
  it("maps executor completion back to the existing subagent result contract", async () => {
    ...
    const runtime = new AgentRuntime(createRegistry(), model, new ToolResolver([]));
  ...
  it("preserves the aborted subagent result contract", async () => {
    ...
    expect(events.map((event) => event.type)).toEqual(["run_started", "run_aborted"]);
  });
  ...
  it("uses isolated model thread ids for parallel subagents", async () => {
    ...
    expect(
      threadIds.every((threadId) => threadId.startsWith("thread-parent/agents/test-agent/run_")),
    ).toBe(true);
    expect(prompts).toEqual(
      expect.arrayContaining(["task A explicit context", "task B explicit context"]),
    );
