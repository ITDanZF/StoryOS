...
describe("ProjectApplication", () => {
  ...
  it("rejects a rename when the target directory already exists", () => {
    ...
    expect(() => app.renameProject({ projectPath: project.path, name: "Occupied" })).toThrow(
      `Project path already exists: ${occupiedPath}`,
    );
    expect(existsSync(project.path)).toBe(true);
  });
  ...
  it("creates project-local StoryOS state and keeps a stable project id after rename", () => {
    ...
    const renamedMetadata = JSON.parse(
      readFileSync(path.join(result.project.path, ".storyos", "project.json"), "utf8"),
    ) as { projectId: string };
  ...
  it("rejects a resource directory that does not exist", () => {
    ...
    expect(() => app.createProject({ name: "Story", parentPath: missingPath })).toThrow(
      `Project resource directory does not exist: ${missingPath}`,
    );
  });
});
