...
describe("project workspace isolation", () => {
  it("creates a hidden system workspace for unowned conversations", () => {
    ...
    expect(
      existsSync(
        path.join(snapshot.systemWorkspace.path, ".storyos", "conversations", "index.json"),
      ),
    ).toBe(false);
  });
  ...
  it("blocks file tools from sibling projects and internal state", () => {
    ...
    expect(() => context.paths.resolve(path.join(root, "ProjectB", "secret.md"))).toThrow(
      "outside the active project workspace",
    );
    expect(() => context.paths.resolve(".storyos/project.json")).toThrow(
  ...
  it("keeps workspace roots and read-before-write state isolated", async () => {
    ...
    contextA.files.remember(fileA, "chapter", statSync(fileA).mtimeMs, false);
    await expect(contextA.files.assertFreshForWrite(fileA, "chapter")).resolves.toBeUndefined();
    await expect(sameRootContext.files.assertFreshForWrite(fileA, "chapter")).rejects.toThrow(
