...
import AgentOrchestrator, {
  ...
  type PlanScheduler,
} from "../../src/main/agent/orchestration/AgentOrchestrator.ts";
import { createAgentOrchestrator } from "../../src/main/story/integration/createStoryAgentOrchestrator.ts";
import type {
  OrchestrationEvent,
  PlannedExecutionPlan,
} from "../../src/main/agent/orchestration/contracts.ts";
import type { PlanProvider } from "../../src/main/agent/orchestration/ports.ts";
...
describe("AgentOrchestrator behavior", () => {
  it("routes direct plans to the direct runner without invoking the scheduler", async () => {
    ...
    expect(directRunner.run).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({ requiredEffects: [] }),
    );
  ...
  it("routes planned work to the scheduler and emits its synthesized answer once", async () => {
    ...
    await expect(
      orchestrator.run(turnInput("analyze and review"), createOptions(events, chunks)),
  ...
  it("forwards cancellation to an active direct run", async () => {
    ...
    const run = orchestrator.run(turnInput("answer directly"), createOptions([], []));
  ...
  it("runs planner, task agent, reviewer, and synthesizer with a fake model", async () => {
    ...
    await expect(
      orchestrator.run(turnInput("analyze text"), createOptions(events, chunks)),
