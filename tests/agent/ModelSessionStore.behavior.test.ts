...
import { DEFAULT_RUN_LIMITS } from "../../src/main/agent/runtime/RunLimits.ts";
import Memory from "../../src/main/agent/checkpoints/index.ts";
import type { ModelSessionStore } from "../../src/main/agent/model/ModelSessionStore.ts";
import { baseSystemPrompt } from "../../src/main/story/resources/prompts/baseSystem.prompt.ts";
...
describe("Model session store behavior", () => {
  ...
  it("uses an injected session store without taking ownership of its lifecycle", async () => {
    ...
    const sessions: ModelSessionStore = {
      getCheckpointer: vi.fn(() => checkpointer) as unknown as ModelSessionStore["getCheckpointer"],
    ...
    expect(langChainMocks.createAgent).toHaveBeenCalledWith(
