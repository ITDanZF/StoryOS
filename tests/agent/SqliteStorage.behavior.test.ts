...
describe("SQLite storage", () => {
  it("preserves an empty conversation workspace without creating a placeholder thread", () => {
    ...
    const databasePath = path.join(root, ".storyos", "project.sqlite");
    ...
    expect(new ThreadApplication(new SqliteThreadStore(reopened.handle)).getSnapshot()).toEqual({
      activeThreadId: null,
      activeThread: null,
      threads: [],
    });
  ...
  it("persists active threads, ordered messages, and normalized skill state", () => {
    ...
    expect(threads.listMessages(created.id).map((message) => message.content)).toEqual([
      "first",
      "second",
    ]);
    ...
    const reopened = new ProjectDatabase(databasePath);
    const restored = new ThreadApplication(new SqliteThreadStore(reopened.handle));
    expect(restored.getActiveThreadId()).toBe(created.id);
    expect(restored.listMessages(created.id)).toHaveLength(2);
    reopened.close();
  });

  it("keeps book registration independent when a project is removed", () => {
    const root = createRoot();
    const database = new ApplicationDatabase(root);
    const projects = new SqliteProjectStore(database.handle);
    ...
    const book = books.registerBookForProject({
      id: "book_00000000-0000-4000-8000-000000000001",
      projectId: "prj-book-binding",
      storagePath: path.join(root, "library", "books", "book-1"),
    });

    expect(books.getBookForProject("prj-book-binding")?.id).toBe(book.id);
    projects.removeProject(projectPath);
    expect(books.getBookForProject("prj-book-binding")).toBeNull();
    expect(books.getBookById(book.id)?.id).toBe(book.id);
    database.close();
  });

  it("separates project and book database schemas", () => {
    const root = createRoot();
    const projectDatabase = new ProjectDatabase(path.join(root, "project.sqlite"));
    const bookDatabase = new BookDatabase(path.join(root, "book.sqlite"));
    const tableNames = (database: ProjectDatabase | BookDatabase) =>
      (
        database.handle
          .prepare(
            `
    ...
    expect(tableNames(projectDatabase)).not.toContain("books");
    expect(tableNames(bookDatabase)).toContain("books");
    expect(tableNames(bookDatabase)).not.toContain("threads");
    bookDatabase.close();
  ...
  it("derives and persists a title from the first user prompt", () => {
    ...
    expect(threads.getSnapshot().activeThread?.title).toBe("检查第五章的节奏，并给出三条建议");
    ...
    const reopened = new ProjectDatabase(databasePath);
    expect(
      new ThreadApplication(new SqliteThreadStore(reopened.handle)).getSnapshot().activeThread
        ?.title,
  ...
  it("restores run summaries and aborts interrupted runs", async () => {
    const root = createRoot();
    const database = new ProjectDatabase(path.join(root, ".storyos", "project.sqlite"));
