...
} from "../../src/main/agent/runtime/AgentExecutor.ts";
import { createRootExecutionContext } from "../../src/main/agent/runtime/ExecutionContext.ts";
...
describe("AgentExecutor behavior", () => {
  ...
  it("uses text invocation for subagent-style execution when available", async () => {
    ...
    await expect(executor.execute(createInput(events, { mode: "text" }))).resolves.toEqual({
      status: "completed",
  ...
  it("returns partial content and emits one failure event when the model fails", async () => {
    ...
    await expect(executor.execute(createInput(events))).resolves.toEqual({
      status: "failed",
  ...
  it("does not emit text deltas produced after cancellation", async () => {
    ...
    expect(events.map((event) => event.type)).toEqual(["run_started", "text_delta", "run_aborted"]);
