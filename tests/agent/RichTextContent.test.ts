...
describe("rich text chapter content", () => {
  ...
  it("reads legacy unversioned documents and rejects future versions", () => {
    expect(parseTiptapDocument('{"type":"doc","content":[{"type":"paragraph"}]}')).toMatchObject({
      type: "doc",
    });
  ...
  it("rejects malformed Tiptap documents", () => {
    expect(() => parseTiptapDocument('{"type":"paragraph"}')).toThrow("root");
    expect(() => parseTiptapDocument('{"type":"doc","content":"bad"}')).toThrow("content");
  });
});
