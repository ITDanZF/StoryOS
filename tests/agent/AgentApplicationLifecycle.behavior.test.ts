...
import type { RunSnapshot } from "../../src/shared/contracts/conversations/applicationContracts.ts";
import type { AgentRunner } from "../../src/main/story/application/conversations/agentPorts.ts";
import type { ApplicationEventRecorder } from "../../src/main/story/application/conversations/runPorts.ts";
...
describe("AgentApplication lifecycle behavior", () => {
  it("restores history and retains only the newest configured runs", async () => {
    ...
    const application = new AgentApplication(runner, {
      initialRuns: [completedHistory(3), completedHistory(2), completedHistory(1)],
      maxRetainedRuns: 2,
    });

    expect(application.listRuns().map((run) => run.runId)).toEqual(["history-3", "history-2"]);
  ...
  it("evicts old settled runs without evicting an active run", async () => {
    ...
    const runner: AgentRunner = {
      run: (input) =>
        input.message.content === "active" ? activePromise : Promise.resolve(input.message.content),
      cancelRun: () => false,
    };
    ...
    const completedRunId = application.startRun(runRequest("completed-thread", "completed"));
  ...
  it("cancels active work, closes logs, and rejects new work during shutdown", async () => {
    ...
    expect(() => application.startRun(runRequest("thread-new", "new work"))).toThrow(
      "shutting down",
    );
    await application.shutdown();
    expect(recorder.close).toHaveBeenCalledOnce();
