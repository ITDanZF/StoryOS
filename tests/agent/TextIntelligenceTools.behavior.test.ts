...
describe("extended general text tool behavior", () => {
  ...
  it("merges sources and revision-checks an existing output", async () => {
    ...
    expect(readFileSync(path.join(root, "merged.txt"), "utf8")).toBe("替换内容");
  });
  ...
  it("inspects UTF-16 text without misclassifying it as binary", async () => {
    ...
    writeFileSync(
      ...
      Buffer.concat([Buffer.from([0xff, 0xfe]), Buffer.from("你好\r\n", "utf16le")]),
    );
    const result = parseResult<{
  ...
  it("returns positioned structural nodes and heading relationships", async () => {
    ...
    const headings = result.result.nodes.filter((node) => node.type === "heading");
  ...
  it("applies input-aware permissions to new mutating tools", () => {
    ...
    expect(policy.getPermission("replace_text", { text: "value" })).toBe("allow");
    expect(policy.getPermission("replace_text", { path: "draft.txt" })).toBe("ask");
