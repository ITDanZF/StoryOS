import SqliteCheckpointRecovery from "../../src/main/agent/checkpoints/SqliteCheckpointRecovery.ts";
...
function checkpoint(threadId: string, checkpointId: string, marker: string): FakeCheckpointRow {
  return {
    thread_id: threadId,
    checkpoint_ns: "",
...
function write(threadId: string, checkpointId: string, marker: string): FakeWriteRow {
  return {
    thread_id: threadId,
    checkpoint_ns: "",
    checkpoint_id: checkpointId,
...
describe("Agent checkpoint rollback behavior", () => {
  it("restores the previous thread tree after a failed run", async () => {
    ...
    const state: FakeDatabaseState = {
      checkpoints: [
        ...
        checkpoint("thread-a/agents/text-analyzer/run-old", "checkpoint-old-child", "old-child"),
        checkpoint("thread-b", "checkpoint-sibling", "sibling"),
      ],
      writes: [
        write("thread-a", "checkpoint-old", "old-root"),
        write("thread-a/agents/text-analyzer/run-old", "checkpoint-old-child", "old-child"),
    ...
    const application = new AgentApplication(runner, {
      checkpointRecovery: new SqliteCheckpointRecovery(checkpointPath),
    });

    const runId = application.startRun(runRequest("thread-a", "run a failing task"));
  ...
  it("allows only one active run per thread while keeping threads independent", async () => {
    ...
    const runner: AgentRunner = {
      run: (input) => runs.get(input.message.content) ?? Promise.reject(new Error("Unknown run")),
      cancelRun: () => false,
    };
    const application = new AgentApplication(runner);

    const firstRunId = application.startRun(runRequest("thread-a", "first"));
    expect(() => application.startRun(runRequest("thread-a", "duplicate"))).toThrow(
