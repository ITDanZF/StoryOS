...
export default function ConversationPage() {
  ...
  return (
    <section className="m-0 flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden border-0 border-border bg-card sm:m-1.5 sm:rounded-xl sm:border lg:ml-2 2xl:mr-3">
      <header className="flex min-h-13 items-center justify-between gap-3 border-b border-border bg-card/95 px-2 [-webkit-app-region:drag] sm:min-h-14 sm:px-3 lg:px-5 2xl:min-h-16 2xl:px-7">
        <div className="flex min-w-0 items-center gap-1 [-webkit-app-region:no-drag]">
          <button className="grid size-8 shrink-0 place-items-center rounded-lg border-0 bg-transparent hover:bg-muted lg:hidden" type="button" aria-label="打开侧栏" onClick={openSidebar}>
            <Menu size={19} />
          </button>
          <button className="flex min-w-0 max-w-[52vw] items-center gap-1.5 rounded-lg border-0 bg-transparent px-1.5 py-1.5 text-xs font-semibold sm:text-[13px] lg:max-w-lg" type="button">
            ...
            <ChevronDown className="shrink-0 text-text-subtle" size={15} />
          </button>
        </div>
        <div className="flex items-center gap-1.5 [-webkit-app-region:no-drag]">
          {globalActiveRun && <span className="hidden h-7 items-center gap-1.5 rounded-full bg-accent px-2.5 text-[10px] text-accent-foreground sm:flex"><Sparkles size={14} />AI 正在回复</span>}
          <span className="hidden h-7 items-center gap-1.5 rounded-full bg-muted px-2.5 text-[10px] text-muted-foreground md:flex">
            <span className={`size-1.5 rounded-full ${state.status?.initialized ? "bg-success-text" : "bg-text-subtle"}`} />
            {state.status?.initialized ? "已连接" : "未配置"}
          </span>
          <button className="grid size-8 place-items-center rounded-lg border-0 bg-transparent hover:bg-muted" type="button" title="模型配置" aria-label="模型配置" onClick={() => navigate("/settings")}>
            <Settings2 size={18} />
          </button>
        </div>
      </header>
