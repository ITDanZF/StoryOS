...
export default function ExportBookDialog({ book, onClose }: { readonly book: ReadyBook; readonly onClose: () => void }) {
  ...
  return <AnimatedDialog onClose={onClose} beforeClose={flow.dispose} busy={busy} stage={stage} className={cn("export-dialog-panel border border-neutral-200 shadow-2xl", flow.state.phase === "choose-format" ? "h-[min(760px,calc(100dvh-24px))] max-w-[980px] rounded-2xl bg-[#f7f7f5]" : successResult ? "max-w-[540px] rounded-[24px] bg-white" : "max-w-[620px] rounded-2xl bg-[#f7f7f5]")} aria-labelledby="export-book-title">
    {({ close }) => <>
      {!successResult && <header className="flex shrink-0 items-start gap-3 border-b border-neutral-200 bg-white px-5 py-4"><span className="grid size-10 place-items-center rounded-xl bg-neutral-900 text-white"><FileOutput size={18} /></span><div className="min-w-0 flex-1"><h2 className="m-0 truncate text-base font-semibold" id="export-book-title">导出《{book.title}》</h2><p className="mb-0 mt-1 text-[11px] leading-5 text-neutral-500">选择用途和格式，StoryOS 会明确说明保留范围。</p></div><button className="grid size-8 place-items-center rounded-lg text-neutral-400 hover:bg-neutral-100 disabled:opacity-40" type="button" disabled={busy} onClick={close} aria-label="关闭"><X size={16} /></button></header>}
      <div key={stage} className={cn("export-dialog-content flex min-h-0 flex-1 flex-col", successResult ? "p-0" : "p-4 sm:p-5")}>
        ...
        {successResult && successVisual && <div className="relative shrink-0 overflow-hidden p-6 sm:p-8">
          ...
          <button className="absolute right-4 top-4 z-10 grid size-8 place-items-center rounded-lg text-neutral-400 transition hover:bg-neutral-100 hover:text-neutral-700" type="button" onClick={close} aria-label="关闭"><X size={16} /></button>
          <div className="motion-reveal relative">
            <span role="status" className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-2.5 py-1 text-[10px] font-semibold text-emerald-700 ring-1 ring-inset ring-emerald-100"><Check className="export-success-check" size={12} strokeWidth={2.5} />导出成功</span>
          ...
          <div className="export-success-file motion-reveal relative mt-6 rounded-2xl border border-neutral-200/80 bg-neutral-50/70 p-4 shadow-[0_1px_2px_rgba(0,0,0,0.03)]">
          ...
          {revealError && <p className="motion-fade relative mt-4 rounded-xl bg-red-50 p-3 text-[11px] text-red-700" role="alert">{revealError}</p>}
          <div className="export-success-actions motion-reveal relative mt-6 flex flex-wrap items-center justify-end gap-2">
            <button className="h-10 rounded-xl border border-neutral-200 bg-white px-4 text-[10px] font-semibold text-neutral-600 transition hover:bg-neutral-50 hover:text-neutral-900" type="button" onClick={close}>完成</button>
            <button className="inline-flex h-10 min-w-[150px] items-center justify-center gap-2 rounded-xl bg-neutral-900 px-5 text-[10px] font-semibold text-white shadow-sm transition hover:bg-neutral-800 disabled:opacity-60" type="button" disabled={revealingFile} onClick={() => void revealFile()}>{revealingFile ? <LoaderCircle className="animate-spin" size={14} /> : <FolderOpen size={14} />}{revealingFile ? "正在打开文件夹" : "在文件夹中显示"}</button>
          </div>
        </div>}
