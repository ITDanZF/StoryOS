...
export default forwardRef<ReaderStageHandle, Props>(function ReaderStage(props, ref) {
  ...
  const surfaceKey = (leafId: string, side: number) => `${leafId}:${props.preferences.theme}:${props.preferences.fontSize}:${props.preferences.lineHeight}:${props.size.width}x${props.size.height}:${side}:${rasterScale}`;
  const keyRef = useRef(surfaceKey); keyRef.current = surfaceKey;
  const profile = `${props.size.width}:${props.size.height}:${props.scale}:${props.double}:${props.preferences.fontSize}:${props.preferences.lineHeight}:${props.preferences.theme}:${props.reduced}:${rasterScale}`;
