...
const inputClass = "w-full rounded-lg border border-border bg-card px-3 py-2 text-sm focus:border-border-strong focus:outline-none disabled:bg-surface-subtle disabled:text-muted-foreground";
...
export default function RecordEditor({ schema, editor, busy, editable, error, onSave, onClose, onDelete, onDirtyChange }: {
  ...
  return (
    <dialog ref={dialogRef} aria-labelledby="record-title" onCancel={(event) => { event.preventDefault(); if (!busy) onClose(); }} className="fixed inset-0 m-0 h-dvh max-h-none w-screen max-w-none bg-transparent p-0 pt-8 backdrop:bg-black/20">
      <aside className="ml-auto flex h-full w-full max-w-xl flex-col border-l border-border bg-card shadow-2xl">
        <header className="flex items-center justify-between border-b border-border px-6 py-5">
          <div><h2 id="record-title" className="font-semibold">{editor.row ? editable ? "编辑记录" : "查看记录" : "新增记录"}</h2><p className="mt-1 font-mono text-xs text-muted-foreground">{schema.name}</p></div>
          <button autoFocus aria-label="关闭记录" disabled={busy} onClick={onClose} className="rounded-lg p-2 hover:bg-muted"><X size={18} /></button>
        </header>
        <form id="record-form" onSubmit={(event) => { event.preventDefault(); submit(); }} className="min-h-0 flex-1 space-y-5 overflow-y-auto p-6">
          {error && <p role="alert" className="rounded-lg bg-danger-surface p-3 text-sm text-danger-text">{error}</p>}
          {!editable && <p className="text-sm text-muted-foreground">当前为浏览模式，开启编辑会话后可修改数据。</p>}
          {schema.columns.map((column, index) => {
            ...
            return <fieldset key={column.name} disabled={disabled} className="space-y-2">
              ...
              {field.mode === "value" && field.type === "blob" && <p className="text-xs text-muted-foreground">使用十六进制字节，例如 FF00A1；空内容代表空 BLOB。</p>}
              {field.mode === "default" && <p className="break-all text-xs text-muted-foreground">{column.defaultSql === null ? "省略此列，由 SQLite 决定是否允许为空或自动生成。" : `默认表达式：${column.defaultSql}`}</p>}
            </fieldset>;
          })}
        </form>
        <footer className="flex justify-between gap-3 border-t border-border p-5">
          <div>{editable && editor.row && <button disabled={busy} onClick={onDelete} className="inline-flex items-center gap-2 rounded-lg px-3 py-2 text-sm text-danger-text hover:bg-danger-surface"><Trash2 size={15} />删除记录</button>}</div>
          <div className="flex gap-2"><button disabled={busy} onClick={onClose} className="rounded-lg border border-border px-4 py-2 text-sm">关闭</button>{editable && <button type="submit" form="record-form" disabled={busy || Boolean(editor.row && !dirty)} className="inline-flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm text-primary-foreground disabled:opacity-40"><Save size={15} />保存记录</button>}</div>
        </footer>
      </aside>
    </dialog>
  );
