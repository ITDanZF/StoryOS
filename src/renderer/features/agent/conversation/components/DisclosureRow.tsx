...
export default function DisclosureRow({
  ...
  return (
    <section className="min-w-0 text-[12px]">
      <button
        className={cn(
          "group relative flex h-7 w-full min-w-0 items-center gap-2 overflow-hidden rounded-md border-0 bg-transparent px-1.5 text-left",
          expandable ? "cursor-pointer hover:bg-neutral-100/80" : "cursor-default",
          failed ? "text-red-600" : "text-neutral-400",
        )}
        type="button"
        disabled={!expandable}
      ...
      >
        ...
        <strong className={cn("relative shrink-0 font-medium", failed ? "text-red-600" : "text-neutral-500")}>{label}</strong>
        {summary && (
          <>
      ...
      {open && children && (
        <div className="ml-7 mt-1 max-h-72 overflow-auto rounded-xl border border-neutral-200 bg-white p-3 text-[11px] leading-5 text-neutral-600 shadow-sm">
          {children}
        </div>
      )}
    </section>
