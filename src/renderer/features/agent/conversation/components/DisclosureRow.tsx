    ...
    <section className="min-w-0 text-xs leading-5">
      <button
        className={cn(
          ...
          expandable ? "cursor-pointer hover:bg-muted/80" : "cursor-default",
      ...
      >
        ...
        <strong className={cn("relative shrink-0 font-normal", failed ? "text-danger-text" : "text-text-secondary")}>{label}</strong>
        {summary && (
          <>
            <span className="relative size-0.5 shrink-0 rounded-full bg-border-strong" />
            <span className="relative min-w-0 flex-1 truncate text-text-subtle">{summary}</span>
          </>
        )}
        {expandable && <ChevronDown className={cn("relative ml-auto shrink-0 transition-transform", open && "rotate-180")} size={13} />}
      </button>
      {open && children && (
        <div className="ml-[22px] mt-1 max-h-72 overflow-auto rounded-xl border border-border bg-surface-subtle p-3 text-[11px] leading-5 text-text-secondary">
          {children}
        </div>
      )}
    </section>
