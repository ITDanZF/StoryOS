...
export default function PaginatedEditorSurface({
  ...
  return (
    ...
    >
      <div
        className="chapter-pagination-layout-switch sticky top-2 z-40 ml-auto -mb-8 flex w-fit items-center rounded-lg border border-border bg-card/95 p-0.5 shadow-sm backdrop-blur"
        role="group"
        aria-label="页面布局模式"
      >
        <button
          className={cn(
            "grid size-7 place-items-center rounded-md border-0 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-border",
            layoutMode === "horizontal"
              ? "bg-accent text-accent-foreground"
              : "bg-transparent text-text-subtle hover:bg-surface-subtle hover:text-text-secondary",
          )}
          type="button"
          title="横向逐页"
