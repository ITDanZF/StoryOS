...
import ApprovalComposer from "../../agent/conversation/components/ApprovalComposer.tsx";
import type {
    ...
    >
      ...
      <div className="flex h-10 shrink-0 items-center gap-1.5 overflow-x-auto border-b border-border px-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        {!contextEnabled && chapterNumber !== null && (
          <button className="grid size-6 shrink-0 place-items-center rounded-full border border-border bg-card text-text-subtle hover:bg-surface-subtle hover:text-foreground" type="button" aria-label="添加章节上下文" onClick={() => onContextEnabledChange(true)}>
            <Plus size={11} />
          </button>
        )}
      </div>
      ...
      <form className={cn(
        "group/composer mx-3 mb-3 shrink-0 rounded-[18px] border border-border bg-card p-3 shadow-[0_8px_24px_rgba(30,28,20,0.07)] transition focus-within:border-accent-border focus-within:ring-2 focus-within:ring-accent-border/70",
