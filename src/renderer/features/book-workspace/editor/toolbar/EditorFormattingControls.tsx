...
function Popover({ label, icon, children }: PopoverProps) {
  ...
  return (
    <div className="relative shrink-0" ref={rootRef}>
      <button
        className="grid size-8 place-items-center rounded-lg border-0 bg-transparent text-muted-foreground hover:bg-muted hover:text-foreground"
        type="button"
        title={label}
        aria-label={label}
        aria-expanded={open}
      ...
      {open && (
        <div className="absolute left-0 top-[calc(100%+6px)] z-50 rounded-xl border border-border bg-card p-2 shadow-[0_12px_32px_rgba(30,28,20,0.16)]">
...
export default function EditorFormattingControls({
  ...
  return (
    <div className="flex shrink-0 items-center gap-0.5">
      ...
      <button className="grid size-8 place-items-center rounded-lg border-0 bg-transparent text-muted-foreground hover:bg-muted hover:text-foreground" type="button" title="减少缩进" onMouseDown={(event) => event.preventDefault()} onClick={() => editor.commands.adjustParagraphIndent(-2)}><IndentDecrease size={15} /></button>
      <button className="grid size-8 place-items-center rounded-lg border-0 bg-transparent text-muted-foreground hover:bg-muted hover:text-foreground" type="button" title="增加缩进" onMouseDown={(event) => event.preventDefault()} onClick={() => editor.commands.adjustParagraphIndent(2)}><IndentIncrease size={15} /></button>

      <div className="relative">
        <button className={cn("grid size-8 place-items-center rounded-lg border-0 bg-transparent text-muted-foreground hover:bg-muted hover:text-foreground", blockState?.linkActive && "bg-accent text-accent-foreground")} type="button" title="插入或编辑链接 Mod+K" aria-pressed={blockState?.linkActive} onMouseDown={(event) => event.preventDefault()} onClick={() => setLinkOpen((current) => !current)}><Link2 size={15} /></button>
        {linkOpen && (
