...
function ToolbarMenu({
  ...
  return (
    <Select
      ...
      onChange={onSelect}
      preserveSelection
      triggerClassName={cn("h-8 text-xs", widthClassName)}
    />
  );
}
    ...
    >
      <div className="flex min-w-0 flex-1 items-center gap-0.5">
        ...
        <ToolbarButton
          ...
          disabled={!activeEditor}
          icon={<Bold size={15} />}
          label={editorCommandLabel("bold")}
          onRun={() => activeEditor && runEditorCommand(activeEditor, "bold")}
        />
        <ToolbarButton
          active={state?.italic}
          disabled={!activeEditor}
          icon={<Italic size={15} />}
          label={editorCommandLabel("italic")}
          onRun={() => activeEditor && runEditorCommand(activeEditor, "italic")}
        />
        ...
        />
        <ToolbarButton
          disabled={!activeEditor || !state?.canUndo}
          icon={<RotateCcw size={15} />}
          label={editorCommandLabel("undo")}
          onRun={() => activeEditor && runEditorCommand(activeEditor, "undo")}
        />
        <ToolbarButton
          disabled={!activeEditor || !state?.canRedo}
          icon={<Redo2 size={15} />}
          label={editorCommandLabel("redo")}
          onRun={() => activeEditor && runEditorCommand(activeEditor, "redo")}
        />
      </div>
