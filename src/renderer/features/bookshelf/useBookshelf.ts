...
export default function useBookshelf() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      while (true) {
        const page = await getBookshelfApi().getBookshelfBooks({
          after,
          limit: 200,
        });
  ...
  useEffect(() => {
    ...
    const unsubscribe = getBookshelfApi().onEvent((event) => {
  ...
  const createBook = useCallback(
    async (
      ...
      setNotice(null);
      try {
        const result = await getBookshelfApi().createBookshelfBook(input);
        await load();
