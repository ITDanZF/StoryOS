...
import { getBookshelfApi } from "../../../platform/desktop/bookshelfApi.ts";
...
export default function useBookshelfTrash() {
  ...
  const load = useCallback(async () => {
    ...
    setLoadError(null);
    try {
      const result = await getBookshelfApi().getBookshelfTrash();
  ...
  const moveToTrash = useCallback(async (bookId: string) => {
    ...
    setNotice(null);
    try {
      const entry = await getBookshelfApi().moveBookshelfBookToTrash(bookId);
      setEntries((current) => [
        entry,
  ...
  const restore = useCallback(async (entry: BookshelfTrashEntry) => {
    ...
    setNotice(null);
    try {
      const restored = await getBookshelfApi().restoreBookshelfBookFromTrash(
        entry.bookId,
      );
  ...
  const permanentlyDelete = useCallback(async (entry: BookshelfTrashEntry) => {
    ...
    setNotice(null);
    try {
      await getBookshelfApi().permanentlyDeleteBookshelfBook({
        bookId: entry.bookId,
