...
export default function ConfigurationDialog({ status, required, onClose, onConfigure }: ConfigurationDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid items-end bg-black/25 p-0 backdrop-blur-[3px] sm:place-items-center sm:p-6" role="presentation">
      <section className="max-h-[90dvh] w-full overflow-y-auto rounded-t-[18px] border border-border bg-white p-5 shadow-2xl sm:w-[min(500px,100%)] sm:rounded-2xl sm:p-6" role="dialog" aria-modal="true" aria-labelledby="config-title">
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1">
            <span className="mb-3 grid size-10 place-items-center rounded-xl bg-neutral-900 text-white"><Bot size={21} /></span>
            <h2 className="m-0 text-lg font-semibold tracking-tight" id="config-title">{status?.initialized ? "AI 模型配置" : "连接你的 AI"}</h2>
          ...
          {!required && <button className="grid size-8 place-items-center rounded-lg border-0 bg-transparent hover:bg-neutral-100" type="button" aria-label="关闭" onClick={onClose}><X size={17} /></button>}
        </div>
