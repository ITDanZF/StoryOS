...
export function registerDeveloperDatabaseIpc(
  service: DeveloperDatabaseService,
  ...
  ipcMain.handle(DEVELOPER_DATABASE_CHANNEL, (event, method: string, ...args: unknown[]) => {
    ...
    switch (method) {
      ...
      case "queryRows":
        return service.queryRows(args[0] as Parameters<typeof service.queryRows>[0]);
      case "readRow":
        return service.readRow(args[0] as Parameters<typeof service.readRow>[0]);
      case "mutate":
        return service.mutate(args[0] as Parameters<typeof service.mutate>[0]);
