...
export function registerWindowIpc(trusted: (id: number) => boolean = () => false): () => void {
  const requireTrustedWindow = (event: IpcMainEvent | IpcMainInvokeEvent) => {
    if (!trusted(event.sender.id) || event.senderFrame !== event.sender.mainFrame)
      throw new Error("Window IPC sender is not trusted.");
    return requireSenderWindow(event);
  };
  const fileBrowser = new DesktopFileBrowserService();
  const closeWindow = (event: IpcMainEvent) => {
    requireTrustedWindow(event).close();
  };
  ipcMain.handle(WINDOW_IPC_CHANNELS.getState, (event) =>
    getWindowState(requireTrustedWindow(event)),
  );
  ipcMain.handle(
    WINDOW_IPC_CHANNELS.pickDirectory,
    async (event, request?: PickDirectoryRequest) => {
      const title = normalizeDialogTitle(request?.title, "选择文件夹");
      const defaultPath = normalizeDefaultPath(request?.defaultPath);
      const result = await dialog.showOpenDialog(requireTrustedWindow(event), {
        title,
  ...
  ipcMain.handle(WINDOW_IPC_CHANNELS.saveFile, async (event, request: SaveFileRequest) => {
    const result = await dialog.showSaveDialog(requireTrustedWindow(event), {
  ...
  ipcMain.handle(WINDOW_IPC_CHANNELS.fileBrowserLocations, () => fileBrowser.getLocations());
  ipcMain.handle(
    WINDOW_IPC_CHANNELS.fileBrowserDirectory,
    (_event, request: ListFileBrowserDirectoryRequest) => fileBrowser.listDirectory(request),
  );
  ipcMain.handle(
    WINDOW_IPC_CHANNELS.fileBrowserTarget,
    (_event, request: ResolveFileBrowserTargetRequest) => fileBrowser.resolveTarget(request),
  );
  ipcMain.handle(WINDOW_IPC_CHANNELS.revealFile, (_event, filePath: string) =>
    fileBrowser.revealFile(filePath),
  );
  ipcMain.handle(
    WINDOW_IPC_CHANNELS.rememberTransferLocation,
    (_event, fileOrDirectoryPath: string) => fileBrowser.rememberLocation(fileOrDirectoryPath),
  );
  ipcMain.handle(WINDOW_IPC_CHANNELS.minimize, (event) => {
    requireTrustedWindow(event).minimize();
  });
  ipcMain.handle(WINDOW_IPC_CHANNELS.toggleMaximize, (event) => {
    const win = requireTrustedWindow(event);
