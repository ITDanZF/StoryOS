...
export function registerAgentIpc(
  ...
  try {
    ...
    new ConversationIpcController(registrar, () => service.requireController());
    ...
    const onResponse = (event: Electron.IpcMainEvent, response: RendererEditorToolResponse) => {
    ...
    disposers.push(() => ipcMain.removeListener(AGENT_IPC_CHANNELS.editorToolResponse, onResponse));
    return close;
  } catch (error) {
    close();
    throw error;
