...
function getUnixVolumes(): string[] {
  if (process.platform === "win32") return [];
  const candidates = process.platform === "darwin" ? ["/", "/Volumes"] : ["/", "/mnt", "/media"];
...
export default class DesktopFileBrowserService {
  ...
  getLocations(): readonly FileBrowserLocation[] {
    const candidates: FileBrowserLocation[] = [
      ...
      ...[...getWindowsVolumes(), ...getUnixVolumes()].map((absolutePath, index) => ({
        ...
        label:
          process.platform === "win32" ? absolutePath : path.basename(absolutePath) || "根目录",
        absolutePath,
  ...
  listDirectory(request: ListFileBrowserDirectoryRequest): FileBrowserPage {
    ...
    if (!["name", "modifiedAt", "size"].includes(sortBy))
      throw new Error("Invalid file browser sort.");
    const direction = request?.sortDirection ?? "asc";
    if (direction !== "asc" && direction !== "desc")
      throw new Error("Invalid file browser sort direction.");
    const offset = request?.cursor === undefined ? 0 : Number.parseInt(request.cursor, 10);
    if (!Number.isSafeInteger(offset) || offset < 0)
      throw new Error("Invalid file browser cursor.");

    let entries: FileBrowserEntry[];
    try {
      entries = readdirSync(directoryPath, { withFileTypes: true }).map((entry) =>
        createEntry(directoryPath, entry.name),
      );
    } catch (error) {
    ...
    if (extensions) {
      entries = entries.filter(
        (entry) =>
          ...
          (entry.extension !== null && extensions.has(entry.extension)),
      );
    }
    ...
    return Object.freeze({
      directoryPath,
      parentPath:
        path.dirname(directoryPath) === directoryPath ? null : path.dirname(directoryPath),
  ...
  resolveTarget(request: ResolveFileBrowserTargetRequest): FileBrowserTarget {
    ...
    const containsControlCharacter = Array.from(baseName).some(
    ...
    if (WINDOWS_RESERVED_NAME.test(baseName))
      throw new Error("Export file name is reserved by Windows.");
    ...
    if (path.dirname(outputPath) !== directoryPath)
      throw new Error("Export path escapes its directory.");
  ...
  rememberLocation(fileOrDirectoryPath: string): void {
    ...
    const next = [
      directoryPath,
      ...this.readRecentLocations().filter((item) => item !== directoryPath),
    ].slice(0, 5);
