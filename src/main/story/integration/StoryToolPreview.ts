import type { ToolApprovalRequest } from "../../agent/tools/security/ToolPolicy.ts";
...
function describeEditorSelector(value: unknown): string {
  ...
  if (selector.kind === "text") {
    ...
    const expectedCount =
      typeof selector.expected_count === "number" ? selector.expected_count : "?";
...
export function createToolApprovalPreview(request: ToolApprovalRequest): string {
  ...
  if (request.toolName === "replace_book_chapter_text") {
    const previous = typeof input.expected_text === "string" ? input.expected_text : "";
    const next = typeof input.replacement_text === "string" ? input.replacement_text : "";
    return truncate(
      [
        "保存层章节正文替换",
  ...
  if (request.toolName === "rewrite_book_chapter_text") {
    const previous =
      typeof input.expected_current_text === "string" ? input.expected_current_text : "";
