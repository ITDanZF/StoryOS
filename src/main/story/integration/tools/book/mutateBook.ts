...
export function createBookMutationTools(context: BookToolContext) {
  const createBook = tool(
    async ({ title, synopsis = "", status = "planning" }) =>
      result("book_created", context.novels.createNovel({ title, synopsis, status })),
    {
  ...
  const updateVolume = tool(
    async ({ volume_id, title, summary, sort_order }) =>
      result(
        "volume_updated",
  ...
  const updateChapter = tool(
    async ({ chapter_id, volume_id, title, status, sort_order }) =>
      result(
        "chapter_updated",
