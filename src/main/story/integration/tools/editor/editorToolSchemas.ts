import { z } from "zod";
import type { EditorStyleChange, EditorTargetSelector } from "./contracts.ts";
...
export const editorTargetSelectorSchema = z.union([textSelectorSchema, rangesSelectorSchema]);
...
export function toEditorTargetSelector(
  ...
  return {
    ...
    occurrences: value.occurrences === "all" ? { kind: "all" } : value.occurrences,
  };
}
