...
export function createEditorTools(client: RendererEditorToolClient, projectId: string) {
  const getContext = tool(
    async () => JSON.stringify(await client.invoke(projectId, { kind: "get_context" }), null, 2),
    {
      name: "get_active_editor_context",
      description:
        "Read the live unsaved text, selection, version, and page of the active StoryOS chapter editor.",
      schema: z.object({}),
    },
  );
  ...
  const openChapter = tool(
      ...
      ),
    {
      ...
      description:
        "Open a chapter, and optionally one of its pages, in the visible StoryOS editor.",
      schema: z.object({
        chapter_id: z.string().min(1),
  ...
  const managePage = tool(
    async ({ chapter_id, expected_version, action, page_number, target_page_number }) =>
      JSON.stringify(
        await client.invoke(projectId, {
          kind: "page_operation",
          chapterId: chapter_id,
          ...
          action,
          ...(page_number === undefined ? {} : { pageNumber: page_number }),
          ...(target_page_number === undefined ? {} : { targetPageNumber: target_page_number }),
        }),
        null,
        2,
      ),
  ...
  const setStyle = tool(
    async ({ chapter_id, expected_version, style }) =>
      JSON.stringify(
        await client.invoke(projectId, {
          kind: "set_style",
          chapterId: chapter_id,
  ...
  const applyTargetedStyles = tool(
    async ({ chapter_id, expected_version, operations }) =>
      JSON.stringify(
        await client.invoke(projectId, {
