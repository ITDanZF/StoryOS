...
export async function importMarkdownBook(
  ...
  for (const file of files) {
    if (file.name.includes("..") || path.isAbsolute(file.name))
      throw new Error("Unsafe Markdown archive path.");
    const value = await file.async("string");
    totalBytes += Buffer.byteLength(value);
    if (totalBytes > 512 * 1024 * 1024)
      throw new Error("Markdown archive expands beyond the maximum size.");
  ...
  const toChapter = (chapter: { title: string; content: string }, index: number) =>
    Object.freeze({
  ...
  const status = ["planning", "writing", "completed", "archived"].includes(
    ...
    for (const item of orderedSnapshotChapters(snapshot)) {
      if (item.volume && item.volume.title !== previousVolume) {
        ...
        if (options.includeVolumeSummaries && item.volume.summary)
          lines.push(item.volume.summary, "");
