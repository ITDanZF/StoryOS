...
export async function importDocxBook(
  content: Buffer,
  filePath: string,
    ...
    .each((_index, element) => {
      ...
      if (/^h[1-6]$/.test(tag)) {
        ...
        if (VOLUME_PATTERN.test(text)) lines.push(text);
          ...
          lines.push(
            `第${Math.max(lines.filter((line) => CHAPTER_PATTERN.test(line)).length + 1, 1)}章 ${text}`,
          );
        return;
      }
      lines.push(text, "");
  ...
  for (const item of orderedSnapshotChapters(snapshot)) {
    if (item.volume && item.volume.title !== previousVolume) {
      ...
      if (options.includeVolumeSummaries && item.volume.summary) {
        paragraphs.push(
          new Paragraph({ children: [new TextRun({ text: item.volume.summary, italics: true })] }),
        );
      }
