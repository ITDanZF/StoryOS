...
const VOLUME_HEADING =
  /^\s*(第[零〇一二三四五六七八九十百千万两\d]+卷|卷[零〇一二三四五六七八九十百千万两\d]+)(?:[\s\u3000:：·.-]+(.*))?\s*$/i;
const CHAPTER_HEADING =
  /^\s*(第[零〇一二三四五六七八九十百千万两\d]+章|chapter\s+[\divxlcdm]+)(?:[\s\u3000:：·.-]+(.*))?\s*$/i;
...
export function parseStructuredPlainText(value: string, fallbackTitle: string): PortableBookDraft {
  const lines = value
    .replace(/^\uFEFF/, "")
    .replace(/\r\n?/g, "\n")
    .split("\n");
  ...
  const portableVolumes: PortableVolumeDraft[] = volumes.map((volume, index) =>
    Object.freeze({
      key: `volume-${index + 1}`,
      title: volume.title,
      summary: "",
...
function nodeToMarkdown(node: TiptapNode, depth = 0): string {
  ...
  if (node.type === "heading") {
    const level =
      typeof node.attrs === "object" && node.attrs && "level" in node.attrs
  ...
  if (node.type === "listItem")
    return `${"  ".repeat(Math.max(depth - 2, 0))}- ${children.trim()}\n`;
...
export function documentToHtml(document: TiptapDocument): string {
  return (
    document.content
      ?.map((node) => {
        if (node.type === "heading")
          return `<h3>${escapeHtml(documentToPlainText({ type: "doc", content: [node] }))}</h3>`;
        if (node.type === "blockquote")
          return `<blockquote>${escapeHtml(documentToPlainText({ type: "doc", content: [node] }))}</blockquote>`;
