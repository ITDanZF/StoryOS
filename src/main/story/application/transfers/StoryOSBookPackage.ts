...
const PACKAGE_FILES = Object.freeze(["manifest.json", "book.sqlite", "checksums.json"]);

export function sha256(content: Uint8Array): string {
  return createHash("sha256").update(content).digest("hex");
}

export function createStoryOSBookPackage(manifest: StoryOSBookManifest, database: Buffer): Buffer {
...
export function readStoryOSBookPackage(content: Buffer): {
  ...
  if (
    fileNames.length !== PACKAGE_FILES.length ||
