...
function toThreadDto(thread: ThreadRecord): ThreadDto {
  ...
  return Object.freeze({
    ...
    metadata: Object.freeze({
      ...
      disabledSkillIds: state.disabledSkillIds,
    }),
  });
}
...
export default class ThreadApplication {
  ...
  getSnapshot(): ThreadSnapshot {
    ...
    const activeThread = this.activeThreadId ? this.requireThread(this.activeThreadId) : null;
    return Object.freeze({
  ...
  appendMessage(request: AppendMessageRequest): MessageDto {
    const threadId =
      request.threadId === undefined ? this.requireActiveThreadId() : request.threadId.trim();
  ...
  listMessages(threadId?: string): readonly MessageDto[] {
    const resolvedThreadId =
      threadId === undefined ? this.requireActiveThreadId() : threadId.trim();
  ...
  useSkill(skillId: string, threadId?: string): ThreadSkillState {
    ...
    const updated = this.store.updateThreadMetadata(thread.id, {
      ...
      disabledSkillIds: normalizeSkillIds(
  ...
  disableSkill(skillId: string, threadId?: string): ThreadSkillState {
    ...
    const updated = this.store.updateThreadMetadata(thread.id, {
      ...thread.metadata,
      activeSkillIds: normalizeSkillIds(
  ...
  clearSkillState(threadId?: string): ThreadSkillState {
    ...
    return normalizeSkillState(
      this.store.updateThreadMetadata(thread.id, {
        ...thread.metadata,
  ...
  private resolveThreadId(threadId: string | undefined): string {
    return threadId === undefined ? this.requireActiveThreadId() : threadId.trim();
  }
