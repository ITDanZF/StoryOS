...
export default class ProjectArchiveRecoveryService {
  ...
  recoverInterruptedRestores(): void {
    for (const operation of this.archives.listIncompleteOperations()) {
      ...
      try {
        ...
        const archiveLayout = getPublishedProjectArchiveLayout(this.agentHome, archive.id);
        ...
        const existingProject = this.projects
          .getSnapshot()
          .projects.find((candidate) => candidate.id === verified.manifest.project.id);
        if (
          existingProject &&
  ...
  private compensate(
    ...
    const project = archive
      ? this.projects
          .getSnapshot()
          .projects.find((candidate) => candidate.id === archive.sourceProjectId)
      : undefined;
    if (project && path.resolve(project.path) === path.resolve(operation.targetPath)) {
      const binding = this.books.getBookForProject(project.id);
      if (binding) {
        if (operation.bookStrategy === "snapshot" && operation.restoredBookId === binding.id) {
          try {
  ...
  private cleanup(operation: ProjectArchiveOperationRecord, removePublished = true): void {
    rmSync(path.join(getProjectArchiveRestoreRoot(this.agentHome), operation.id), {
      recursive: true,
      force: true,
    });
