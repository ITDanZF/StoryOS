import type { ProjectNavigationSnapshot } from "../../../../shared/contracts/projects/projectNavigationContracts.ts";
...
import ThreadApplication from "../conversations/ThreadApplication.ts";
...
export default class ProjectNavigationReader {
  ...
  read(projectId: string): ProjectNavigationSnapshot {
    const project = this.projects.getSnapshot().projects.find((item) => item.id === projectId);
    if (!project) throw new Error(`Project not found: ${projectId}`);

    const database = new ProjectDatabase(getWorkspaceLayout(project.path).projectDatabasePath);
    ...
    try {
      ...
      bookLease = registeredBook ? this.bookRuntimes.acquire(registeredBook.id) : null;
      const novels = bookLease ? new NovelApplication(bookLease.persistence) : null;
