...
export default class ProjectApplication {
  ...
  getActiveProjectPath(): string | null {
    return this.activeProjectId
      ? (this.store.getProjectById(this.activeProjectId)?.path ?? null)
      : null;
  }

  getSnapshot(): ProjectSnapshot {
    const activeProject = this.activeProjectId
      ? this.store.getProjectById(this.activeProjectId)
      : null;
    return Object.freeze({
  ...
  renameProject(request: RenameProjectRequest): ProjectRenameResult {
    ...
    if (!sameFilesystemPath && existsSync(nextPath))
    ...
    try {
      ...
      if (
        !samePath &&
        existsSync(nextPath) &&
        (sameFilesystemPath || !existsSync(previousProject.path))
      ) {
  ...
  private requireParentDirectory(parentPath: string): void {
    if (!existsSync(parentPath))
      throw new Error(`Project resource directory does not exist: ${parentPath}`);
    if (!statSync(parentPath).isDirectory())
      throw new Error(`Project resource path is not a directory: ${parentPath}`);
  }
}
