...
import type {
  ...
  ThreadPersistence,
  ThreadRecord,
} from "../../application/conversations/threadPorts.ts";
...
export default class SqliteThreadStore implements ThreadPersistence {
  constructor(
    ...
    private readonly currentBook: () => string | null = () => null,
  ) {}
  ...
  createThread(
    ...
    this.database.transaction(() => {
      this.database
        .prepare(
          `
        INSERT INTO threads(id, title, created_at, updated_at,book_id)
        VALUES (?, ?, ?, ?,?)
      `,
        )
        .run(id, title, now, now, this.currentBook());
  ...
  getThread(threadId: string): ThreadRecord | null {
    const row = this.database.prepare("SELECT * FROM threads WHERE id = ?").get(threadId) as
      | ThreadRow
      | undefined;
  ...
  listThreads(): ThreadRecord[] {
    return (
      this.database
        .prepare("SELECT * FROM threads ORDER BY updated_at DESC, id ASC")
        .all() as ThreadRow[]
    ).map((row) => this.toThreadRecord(row));
  }

  touchThread(threadId: string): void {
    const result = this.database
      .prepare("UPDATE threads SET row_version=row_version+1, updated_at = ? WHERE id = ?")
  ...
  updateThreadTitle(threadId: string, title: string): ThreadRecord {
    ...
    const result = this.database
      .prepare(
        "UPDATE threads SET title = ?, row_version=row_version+1, updated_at = ? WHERE id = ?",
      )
  ...
  listMessages(threadId: string): MessageRecord[] {
    ...
    return (
      this.database
        .prepare(
          `
      SELECT id, thread_id, role, blocks, updated_at AS created_at
      FROM message_views
      WHERE thread_id = ?
    ...
    `,
      ...
      role: row.role,
      content: (
        JSON.parse(row.blocks) as {
          type: string;
          channel?: string;
          content?: string;
        }[]
      )
        .filter((b) => b.type === "text" && (!b.channel || b.channel === "answer"))
        .map((b) => b.content ?? "")
        .join("\n"),
    ...
    return {
      ...
      metadata: {
        activeSkillIds: Object.freeze(
          skills.filter((skill) => skill.status === "active").map((skill) => skill.skill_id),
        ),
        disabledSkillIds: Object.freeze(
          skills.filter((skill) => skill.status === "disabled").map((skill) => skill.skill_id),
        ),
      },
    };
  }

  private replaceSkillState(threadId: string, metadata: ThreadMetadata | undefined): void {
    const active = this.normalizeSkillIds(metadata?.activeSkillIds);
    const disabled = this.normalizeSkillIds(metadata?.disabledSkillIds).filter(
      (skillId) => !active.includes(skillId),
    );
    this.database.prepare("DELETE FROM thread_skills WHERE thread_id = ?").run(threadId);
