...
import { pathMatches } from "../../../agent/tools/text/indexing/MemoryTextIndexStore.ts";
import type {
  TextIndexFileState,
  TextIndexSearchOptions,
  TextIndexStore,
} from "../../../agent/tools/text/indexing/TextIndexStore.ts";
import {
  normalizeSearchText,
  tokenizeSearchText,
} from "../../../agent/tools/text/indexing/tokenizer.ts";
import type {
  IndexedTextChunk,
  IndexedTextFile,
  RankedTextHit,
} from "../../../agent/tools/text/indexing/types.ts";
...
export default class SqliteTextIndexStore implements TextIndexStore {
  ...
  updateFileState(state: TextIndexFileState): void {
    ...
    `,
      ...
      .run(state.revision, state.sizeBytes, state.mtimeMs, Date.now(), state.path);
    if (result.changes === 0) {
  ...
  replaceFile(file: IndexedTextFile, state: TextIndexFileState): void {
    this.database.transaction(() => {
      ...
      `,
        ...
        .run(file.path, state.revision, state.sizeBytes, state.mtimeMs, Date.now());
  ...
  deleteFilesNotIn(paths: ReadonlySet<string>): void {
    const existing = this.database.prepare("SELECT path FROM indexed_files").all() as Array<{
      path: string;
    }>;
    this.database.transaction(() => {
  ...
  search(query: string, options: TextIndexSearchOptions = {}): readonly RankedTextHit[] {
    ...
    for (const row of rows) {
      ...
      const normalizedHeading = normalizeSearchText(chunk.headingPath.join(" "));
  ...
  getChunks(paths?: readonly string[], glob?: string): readonly IndexedTextChunk[] {
    ...
    return Object.freeze(
      rows.filter((row) => pathMatches(row.file_path, paths, glob)).map((row) => this.toChunk(row)),
    );
  }
  ...
  getNeighbors(chunk: IndexedTextChunk, radius: number): readonly IndexedTextChunk[] {
    ...
    `,
      ...
      .all(chunk.path, chunk.index - radius, chunk.index + radius, chunk.id) as ChunkRow[];
    return Object.freeze(rows.map((row) => this.toChunk(row)));
  }

  private deleteFile(filePath: string): void {
    this.database.prepare("DELETE FROM text_chunks WHERE file_path = ?").run(filePath);
    this.database.prepare("DELETE FROM indexed_files WHERE path = ?").run(filePath);
  }

  private toChunk(row: ChunkRow): IndexedTextChunk {
    const headingPath = row.heading_path ? row.heading_path.split(HEADING_SEPARATOR) : [];
    return Object.freeze({
      id: row.id,
      path: row.file_path,
      ...
      tokens: tokenizeSearchText(`${headingPath.join(" ")} ${row.content}`),
    });
  }
}
