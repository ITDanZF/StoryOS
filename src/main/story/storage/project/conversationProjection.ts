...
import { deriveThreadTitle, isUntitledThreadTitle } from "../../../../shared/agent/threadTitle.ts";
...
export function projectConversationEvent(database: Database, event: StoredEvent): void {
  ...
  if (user) {
    ...
    const prior = database
      .prepare("SELECT 1 FROM message_views WHERE thread_id=? AND role='user' LIMIT 1")
      .get(event.thread_id);
  ...
  const id = user || standalone ? String(payload.messageId) : `${event.run_id}:assistant`;
  ...
  database
    .prepare("UPDATE threads SET updated_at=?,row_version=row_version+1 WHERE id=?")
...
export function rebuildMessageViews(database: Database, threadId: string): void {
  database.transaction(() => {
    database.prepare("DELETE FROM message_views WHERE thread_id=?").run(threadId);
    let cursor = 0;
    for (;;) {
      const rows = database
        .prepare(
