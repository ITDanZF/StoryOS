...
function removeExactDirectories(parentPath: string, names: readonly string[]): void {
  const resolvedParent = path.resolve(parentPath);
  for (const name of names) {
    const target = path.resolve(resolvedParent, name);
    if (path.dirname(target) !== resolvedParent) {
...
function clearProjectManagedData(stateRoot: string): void {
  ...
  removeExactDirectories(resolvedStateRoot, ["checkpoints", "text-index", "tmp"]);
}
...
function resolveWorkspaceRoots(agentHome: string): readonly string[] {
  const roots = new Set<string>([path.resolve(agentHome, "workSpaceRoot")]);
...
export function prepareLegacyGlobalStorageReset(agentHome: string): LegacyGlobalStorageReset {
  ...
  removeExactFiles(resolvedHome, ["app.sqlite", "app.sqlite-wal", "app.sqlite-shm"]);
...
export function resetLegacyProjectStorage(projectRoot: string): void {
  ...
  const hadLegacyDatabase = ["storyos.sqlite", "storyos.sqlite-wal", "storyos.sqlite-shm"].some(
    (name) => existsSync(path.join(stateRoot, name)),
  );
  removeExactFiles(stateRoot, ["storyos.sqlite", "storyos.sqlite-wal", "storyos.sqlite-shm"]);
