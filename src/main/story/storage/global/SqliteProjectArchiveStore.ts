...
import type {
  ...
  ProjectArchiveState,
} from "../../application/projects/projectArchiveContracts.ts";
import type { ProjectArchiveStore } from "../../application/projects/projectArchivePorts.ts";
...
export default class SqliteProjectArchiveStore implements ProjectArchiveStore {
  ...
  getById(archiveId: string): ProjectArchiveRecord | null {
    const row = this.database.prepare("SELECT * FROM archives WHERE id = ?").get(archiveId) as
      | ArchiveRow
      | undefined;
  ...
  updateOperation(input: {
    ...
    `,
      ...
      .run(
        input.state,
        input.state === "completed" || input.state === "failed" ? input.state : "running",
  ...
  private toRecord(row: ArchiveRow): ProjectArchiveRecord {
    return Object.freeze({
      ...
      restoredAt: row.last_restored_at === null ? null : new Date(row.last_restored_at),
    });
  }

  private toOperationRecord(row: ArchiveOperationRow): ProjectArchiveOperationRecord {
    return Object.freeze({
      id: row.id,
