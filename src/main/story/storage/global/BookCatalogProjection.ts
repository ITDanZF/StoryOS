...
import type { NovelStatus } from "../../application/books/novelPorts.ts";
...
export default class BookCatalogProjection {
  ...
  get deviceId(): string {
    return (
      this.app.prepare("SELECT device_id FROM app_state WHERE singleton=1").get() as {
        device_id: string;
      }
    ).device_id;
  }
  ...
  refresh(bookId: string, source: Database): void {
    const snapshot = source.transaction(() => {
      const book = source.prepare("SELECT id,title,synopsis,status,updated_at FROM books").get() as
        | {
            id: string;
            title: string;
            synopsis: string;
        ...
        | undefined;
      if (!book || book.id !== bookId) throw new Error(`Book identity mismatch: ${bookId}`);
      const stats = source
        .prepare(
    ...
    this.app.transaction(() => {
      ...
      this.app
        ...
        .run(
          ...
          stats.chapters,
          stats.characters,
          Math.max(book.updated_at, stats.updated, volumes.updated, changes.updated),
