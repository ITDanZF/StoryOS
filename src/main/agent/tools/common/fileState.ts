...
export default class FileStateTracker {
  ...
  remember(absolutePath: string, content: string, mtimeMs: number, partial: boolean): void {
  ...
  async assertFreshForWrite(absolutePath: string, currentContent: string): Promise<void> {
    ...
    if (state.partial) {
      throw new Error(
        "Only part of the file was read. Read the whole file before editing or overwriting it.",
      );
    }
    ...
    if (currentMtimeMs > state.mtimeMs && currentContent !== state.content) {
      throw new Error(
        "File changed after it was read. Read it again before editing or overwriting it.",
      );
    }
  }
}
