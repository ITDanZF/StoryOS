...
export function createSearchTextTool(context: WorkspaceToolContext) {
  return tool(
    async ({
      pattern,
      path: searchPath,
      ...
      const absolutePath = await context.paths.resolveExisting(searchPath);
      ...
      for (const file of files) {
        ...
        const relativePath = context.paths.toRelative(file);
        if (
          fileMatcher &&
