import { DynamicStructuredTool, isStructuredTool, type ClientTool } from "@langchain/core/tools";
...
function summarizeInput(toolName: string, input: unknown): string {
  ...
  const outputPath = typeof values.output_path === "string" ? values.output_path : undefined;
...
async function emit(handler: GuardToolsOptions["onEvent"], event: ToolExecutionEvent) {
  await handler?.(event);
}
    ...
    return new DynamicStructuredTool({
      ...
      func: async (input, _runManager, config) => {
        ...
        const request: ToolApprovalRequest = Object.freeze({
          ...
          summary:
            policy.summarizeInput?.(originalTool.name, input) ??
            summarizeInput(originalTool.name, input),
          input,
        });
