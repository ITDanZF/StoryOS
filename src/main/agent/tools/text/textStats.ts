...
export function estimateTokenCount(content: string): number {
  const cjkCharacters = countMatches(
    content,
...
export function calculateTextStats(content: string) {
  ...
  const sentences = content.trim()
    ? content.split(/[。！？!?]+(?:["'”’」』】）)]*)\s*/u).filter((item) => item.trim()).length
    : 0;
  ...
  return Object.freeze({
    ...
    average_paragraph_characters:
      paragraphLengths.length === 0
        ? 0
        : Math.round(
            paragraphLengths.reduce((total, length) => total + length, 0) / paragraphLengths.length,
          ),
