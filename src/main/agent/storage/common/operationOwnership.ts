...
export function markOperationDirectory(
  root: string,
  operationId: string,
): void {
  writeFileSync(
...
export function ownsOperationDirectory(
  root: string,
  operationId: string,
): boolean {
  ...
  if (normalize(realpathSync(root)) !== normalize(path.resolve(root)))
    return false;
  try {
    const marker = JSON.parse(
      readFileSync(path.join(root, markerName), "utf8"),
    );
...
export function removeOperationDirectory(
  root: string,
  operationId: string,
): void {
