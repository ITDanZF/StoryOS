...
export default class SqliteProjectStore implements ProjectStore {
  ...
  upsertProject(input: {
    ...
    const transaction = this.database.transaction(() => {
      const existing = this.database
        .prepare("SELECT * FROM projects WHERE id = ? OR path_key = ? LIMIT 1")
        .get(input.id, pathKey) as ProjectRow | undefined;
      if (existing) {
        if (existing.id !== input.id)
          throw new Error("Project path belongs to another identity.");
        this.database
          .prepare(
            `
          UPDATE projects
          SET id = ?, local_path = ?, path_key = ?, name = ?, location_type = ?,
              trusted = ?, row_version=row_version+1, updated_at = ?, last_opened_at = ?
          WHERE id = ?
        `,
          )
          .run(
        ...
        this.database
          ...
          INSERT INTO projects (
            id, local_path, path_key, name, location_type, trusted,
  ...
  renameProject(
    ...
    if (!existing)
      throw new Error(`Project not found: ${path.resolve(projectPath)}`);
    ...
    try {
      this.database
        .prepare(
          `
        UPDATE projects
        SET local_path = ?, path_key = ?, name = ?, row_version=row_version+1, updated_at = ?, last_opened_at = ?
        WHERE id = ?
      `,
        )
        .run(
  ...
  listProjects(): ProjectRecord[] {
    return (
      this.database
        .prepare("SELECT * FROM projects ORDER BY last_opened_at DESC, id ASC")
        .all() as ProjectRow[]
  ...
  removeProject(projectPath: string): void {
    this.database
      .prepare("DELETE FROM projects WHERE path_key = ?")
  ...
  restoreProject(input: ProjectRecord): ProjectRecord {
    ...
    const transaction = this.database.transaction(() => {
      ...
      if (conflict) {
        throw new Error(
          `Project restore conflicts with an existing project: ${input.id}`,
        );
      }
      this.database
        .prepare(
  ...
  getActiveProjectId(): string | null {
    const row = this.database
      .prepare("SELECT active_project_id FROM app_state WHERE singleton = 1")
  ...
  setActiveProjectId(projectId: string | null): void {
    ...
    this.database
      .prepare("UPDATE app_state SET active_project_id = ? WHERE singleton = 1")
      .run(projectId);
  }
