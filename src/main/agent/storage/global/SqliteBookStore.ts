...
export default class SqliteBookStore implements BookRegistry {
  ...
  detachBook(projectId: string): void {
    this.database
      .prepare("DELETE FROM project_books WHERE project_id = ?")
      .run(projectId);
  }
  ...
  moveBookToTrash(input: {
    ...
    return this.database.transaction(() => {
      ...
      if (book.state !== "available") {
        throw new Error(
          `Only available book_registry can be trashed: ${input.bookId}`,
        );
      }
      ...
      if (!catalog)
        throw new Error(
          "Book catalog must be refreshed before trashing a book.",
        );
      this.database
        .prepare(
  ...
  restoreBookFromTrash(
    ...
    return this.database.transaction(() => {
      ...
      this.database
        .prepare(
          `UPDATE book_registry SET state=?,trashed_at=NULL,updated_at=? WHERE book_id=?`,
        )
        .run(state, Date.now(), bookId);
      return this.requireBook(bookId);
    })();
  ...
  beginBookCleanup(input: {
    ...
    this.database.transaction(() => {
      ...
      this.database
        .prepare(
          `INSERT INTO book_cleanup_details VALUES (?,?,?,'pending',?,?)`,
        )
        .run(
          input.operationId,
          input.bookId,
  ...
  deleteBookRegistration(input: {
    ...
    this.database.transaction(() => {
      ...
      const operation = this.database
        .prepare(
          `SELECT book_id FROM book_cleanup_details WHERE operation_id=?`,
        )
        .get(input.operationId) as { book_id: string } | undefined;
      if (!operation || operation.book_id !== input.bookId)
        throw new Error(
          "Book cleanup must be prepared before deleting registration.",
        );
      this.database
        .prepare(
  ...
  updateBookDeletionCleanup(
    ...
    this.database.transaction(() => {
      ...
      this.database
        .prepare(
          `UPDATE storage_operations SET state=?,phase=?,updated_at=? WHERE id=?`,
        )
  ...
  abandonImportedBook(bookId: string): void {
    const result = this.database
      .prepare(
        "DELETE FROM book_registry WHERE book_id = ? AND state = 'importing'",
      )
      .run(bookId);
    if (result.changes !== 1) {
  ...
  rollbackRestoredBook(input: {
    ...
    this.database.transaction(() => {
      const book = this.requireBook(input.bookId);
      if (
        storagePathKey(book.storagePath) !== storagePathKey(input.storagePath)
      ) {
  ...
  private toRecord(row: BookRow): BookRecord {
    return Object.freeze({
      ...
      lastOpenedAt:
        row.last_opened_at === null ? null : new Date(row.last_opened_at),
    });
  }
}
