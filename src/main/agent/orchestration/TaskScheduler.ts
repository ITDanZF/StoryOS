...
import { createRunAbortScope, RunBudgetExceededError } from "../runtime/RunLimits.ts";
import type {
  ApprovedTaskResult,
...
import type { AnswerSynthesisProvider, PlannedTaskRunner, ResultReviewProvider } from "./ports.ts";
...
function describeTaskFailure(
  ...
  const findings = review.findings
    ...
    .filter(Boolean);
  const details = [...new Set([...(result.error ? [result.error] : []), ...findings])];
  const reason =
    details.length > 0
      ? details.join("；")
