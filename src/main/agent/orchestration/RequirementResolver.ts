...
import type { CapabilityId, EffectId } from "../runtime/capabilities.ts";
import type { ExecutionRequirements } from "./contracts.ts";

const WRITE_PATTERN =
  /(?:完成|创作|续写|继续|接着写|继续写|往下写|重新继续|生成|创建|修改|更新|删除|重写|改写|填充|补全|写入|write|continue|create|update|delete|rewrite|generate|complete)/i;
...
const ANALYZE_PATTERN =
  /(?:分析|总结|提取|比较|归纳|动机|结构|analy[sz]e|summari[sz]e|extract|compare)/i;
