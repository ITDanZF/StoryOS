import type { AgentTurnInput } from "../runtime/AgentInput.ts";
import type { ToolApprovalHandler } from "../tools/security/ToolPolicy.ts";
import type { AgentEventHandler } from "../runtime/AgentEvent.ts";
import type { EffectId } from "../runtime/capabilities.ts";
import RunBudget, {
  createRunAbortScope,
  DEFAULT_RUN_LIMITS,
  RunTimedOutError,
...
export default class AgentOrchestrator {
  ...
  async run(input: AgentTurnInput, options: AgentOrchestratorRunOptions): Promise<string> {
  ...
  cancelRun(runId: string, reason: unknown = new Error("Agent orchestration cancelled.")): boolean {
