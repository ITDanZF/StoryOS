...
export const executionRequirementsSchema = z
  .object({
    capabilities: z.array(z.string().regex(/^[a-z][a-z0-9_-]*(?:\.[a-z][a-z0-9_-]*)+$/)).min(1),
    effects: z.array(z.string().regex(/^[a-z][a-z0-9_-]*(?:\.[a-z][a-z0-9_-]*)+$/)),
    contextKinds: z.array(z.string().trim().min(1)).min(1),
...
export const proposedPlanDraftSchema = z
  .object({
    version: z.literal(2),
