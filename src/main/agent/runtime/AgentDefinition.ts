...
export function validateAgentDefinition(definition: AgentDefinition): void {
  ...
  for (const toolName of definition.allowedToolIds) {
    ...
    if (toolNames.has(toolName)) {
      throw new Error(`Duplicate agent tool ${toolName}: ${definition.id}`);
    }

    toolNames.add(toolName);
  }

  if (definition.model !== undefined && !definition.model.trim()) {
  ...
  if (!Number.isInteger(definition.limits.maxTurns) || definition.limits.maxTurns <= 0) {
    throw new Error(`Agent maxTurns must be a positive integer: ${definition.id}`);
  }
}
...
export function defineAgent(definition: AgentDefinition): AgentDefinition {
  ...
  const metadata = definition.metadata ? Object.freeze({ ...definition.metadata }) : undefined;
