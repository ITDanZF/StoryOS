...
import AgentExecutor, { type AgentModelRunner } from "./AgentExecutor.ts";
...
export default class AgentRuntime {
  ...
  constructor(
    ...
    private readonly toolPolicy: ToolPolicy = new ToolPolicy((id) =>
      toolResolver.registry.getManifest(id),
    ),
  ...
  async run(input: RunAgentInput): Promise<AgentRunResult> {
    ...
    const tools = guardTools(this.toolResolver.resolve(input.grantedToolIds), {
      ...
      budget,
      onEvent: (event) => emitToolExecutionEvent(input.onEvent, context, event),
    });
