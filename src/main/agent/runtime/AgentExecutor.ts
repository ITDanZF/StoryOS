...
import { createAgentEvent, emitAgentEvent, type AgentEventHandler } from "./AgentEvent.ts";
...
export default class AgentExecutor {
  ...
  constructor(model: AgentModelRunner | ModelRouter) {
    this.modelRouter = model instanceof ModelRouter ? model : new ModelRouter(model);
  }
  ...
  async execute(input: AgentExecutorInput): Promise<AgentExecutionResult> {
    ...
    try {
      const systemPrompt =
        typeof input.systemPrompt === "function" ? await input.systemPrompt() : input.systemPrompt;
      ...
      if (input.mode === "text" && model.invokeText) {
        await this.appendChunk(await model.invokeText(modelInput), input, chunks);
      } else {
        for await (const chunk of model.stream(modelInput)) {
          if (typeof chunk === "string" || chunk.channel === "answer") {
            await this.appendChunk(typeof chunk === "string" ? chunk : chunk.delta, input, chunks);
          } else {
            await emitAgentEvent(
              input.onEvent,
              createAgentEvent(context, {
