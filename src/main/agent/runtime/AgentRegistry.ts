...
export default class AgentRegistry {
  ...
  registerMany(definitions: Iterable<AgentDefinition>): readonly AgentDefinition[] {
  ...
  validateAgainstTools(tools: ToolRegistry): void {
    for (const definition of this.definitions.values()) {
      ...
      if (definition.executionModes.includes("planned") && definition.allowedEffects.length > 0) {
        throw new Error(`Planned agent cannot declare side effects: ${definition.id}.`);
      }
    }
  }
}
