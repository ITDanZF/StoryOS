...
export default class RunBudget {
  ...
  constructor(readonly limits: RunLimits = DEFAULT_RUN_LIMITS) {
    ...
    if (!Number.isInteger(limits.maxDelegationDepth) || limits.maxDelegationDepth < 0) {
...
export function createRunAbortScope(timeoutMs: number, parentSignal?: AbortSignal): RunAbortScope {
