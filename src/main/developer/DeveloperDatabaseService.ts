...
} from "../agent/storage/book/BookDatabase.ts";
...
import {
  ...
  getWorkspaceLayout,
} from "../agent/workspace/ProjectLayout.ts";
...
export default class DeveloperDatabaseService {
  ...
  listTables(id: string) {
    return this.withDatabase(id, (db) => listTables(db));
  }
  ...
  async endEditSession(): Promise<void> {
    ...
    this.transitioning = true;
    try {
      for (const item of this.databases.values()) {
        if (!existsSync(item.path)) continue;
        const db = this.open(item.path, true);
        let appId: number;
        try {
          if ((db.pragma("foreign_key_check") as unknown[]).length)
            throw new Error(`外键校验未通过：${item.path}`);
