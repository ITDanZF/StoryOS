...
const valueSchema = z.discriminatedUnion("type", [
  ...
  z
    .object({
      type: z.enum(["integer", "real", "text", "blob"]),
      value: z.string().max(16 * 1024 * 1024),
    })
    .strict(),
]);
const tableRequest = z.object({
...
const valuesSchema = z
  ...
  .transform((input) =>
    Object.fromEntries(
      Object.entries(input).map(([name, value]) => [name, valueSchema.parse(value)]),
    ),
  );
...
const queryRequest = tableRequest.extend({
  ...
  filter: z
    .object({
      column: z.string(),
      operator: z.enum(["eq", "ne", "contains", "gt", "lt", "null", "notNull"]),
      value: valueSchema,
    })
    .optional(),
});
...
export function bindValue(value: SqlValue): string | number | bigint | Buffer | null {
  ...
  if (value.type === "blob") {
    if (!/^(?:[0-9a-fA-F]{2})*$/.test(value.value))
      throw new Error("BLOB 必须是完整的十六进制字节。");
    return Buffer.from(value.value, "hex");
  }
  if (value.type === "integer") {
    if (!/^-?\d+$/.test(value.value)) throw new Error("请输入有效整数。");
    const result = BigInt(value.value);
    if (result < -(2n ** 63n) || result > 2n ** 63n - 1n)
      throw new Error("整数超出 SQLite 64 位范围。");
    return result;
  }
  if (!value.value.trim() || !Number.isFinite(Number(value.value)))
...
function encode(value: unknown, type: string, hexadecimal = false): SqlValue {
  ...
  if (type === "blob")
    return {
      type: "blob",
      value: hexadecimal ? String(value).toLowerCase() : (value as Buffer).toString("hex"),
    };
  if (type === "text" && hexadecimal)
    return { type: "text", value: Buffer.from(String(value), "hex").toString("utf8") };
...
function schemaFor(db: Database.Database, request: TableRequest): TableSchema {
  ...
  if (schema.version !== request.version)
    throw new Error("表结构已变化，请保留草稿并刷新结构后重试。");
  return schema;
}
function column(schema: TableSchema, name: string): string {
  if (!schema.columns.some((field) => field.name === name))
    throw new Error("字段不存在，请刷新结构。");
  return quote(name);
}
...
function identityWhere(
  ...
  if (!schema.writable || identity.length !== schema.locator.length)
    throw new Error("此记录没有有效行标识。");
  return {
    sql: schema.locator.map((name) => `${quote(name)} IS ?`).join(" AND "),
    params: identity.map(bindValue),
  };
}
function detail(db: Database.Database, schema: TableSchema, identity: SqlValue[]): RowDetail {
  ...
  const sizeSql = schema.columns
    .map(({ name }) => `coalesce(length(cast(${quote(name)} AS blob)),0)`)
    .join(" + ");
  const sizes = db
    .prepare(`SELECT ${sizeSql} FROM ${quote(schema.name)} WHERE ${where.sql} LIMIT 2`)
    .pluck()
    .all(...where.params) as number[];
  if (sizes.length !== 1) throw new Error("记录已删除或行标识不唯一，请刷新。");
  if (sizes[0] > 8 * 1024 * 1024)
    throw new Error("此记录超过 8 MiB 编辑上限，请使用外部数据库工具处理。");
  const raw = db
    .prepare(`SELECT ${projection(schema, false)} FROM ${quote(schema.name)} WHERE ${where.sql}`)
    .raw()
    .safeIntegers()
    .get(...where.params) as unknown[];
  ...
  return db.transaction(() => {
    ...
    if (request.filter) {
      ...
      if (operator === "null" || operator === "notNull")
        where = `${field} IS ${operator === "notNull" ? "NOT " : ""}NULL`;
      else if (operator === "contains") {
        if (value.type !== "text") throw new Error("包含查询需要文本值。");
        where = `instr(cast(${field} AS text), ?) > 0`;
        params.push(value.value);
      } else {
        where = `${field} ${operators[operator]} ?`;
        params.push(bindValue(value));
      }
    }
    const order = request.sort
      ? [`${column(schema, request.sort.column)} ${request.sort.direction.toUpperCase()}`]
      : [];
    order.push(...schema.locator.map(quote));
    const locators = schema.locator
      .map((name) => `, ${quote(name)}, typeof(${quote(name)})`)
      .join("");
    const rows = db
      .prepare(
        `SELECT ${projection(schema, true)}${locators} FROM ${quote(schema.name)}${where ? ` WHERE ${where}` : ""}${order.length ? ` ORDER BY ${order.join(", ")}` : ""} LIMIT ? OFFSET ?`,
      )
      .raw()
      .safeIntegers()
      .all(...params, request.limit + 1, request.offset) as unknown[][];
    return {
      hasMore: rows.length > request.limit,
      rows: rows.slice(0, request.limit).map((raw) => ({
        cells: cells(schema, raw, true),
        truncated: cells(schema, raw, true).map(
          (value, index) =>
            Number(raw[index * 3 + 2]) >
            (value.type === "text"
        ...
        identity: schema.locator.length
          ? schema.locator.map((_, index) =>
              encode(
                raw[schema.columns.length * 3 + index * 2],
                raw[schema.columns.length * 3 + index * 2 + 1] as string,
              ),
            )
          : null,
      })),
...
export function mutate(db: Database.Database, input: MutationRequest): void {
  ...
  db.transaction(() => {
    const schema = schemaFor(db, request);
    if (!schema.insertable) throw new Error(schema.reason ?? "当前表不可写入。");
    const names = Object.keys(request.values);
    const fields = names.map((name) => {
      const result = column(schema, name);
      const field = schema.columns.find((field) => field.name === name);
      if (!field) throw new Error("数据列不存在。");
      if (field.hidden) throw new Error("生成列不可直接写入。");
      return result;
    });
    const values = names.map((name) => bindValue(request.values[name]));
    const byteSize = values.reduce<number>(
      (total, value) =>
        total +
        (typeof value === "string"
    ...
    if (request.operation === "insert") {
      db.prepare(
        `INSERT INTO ${quote(schema.name)} ${fields.length ? `(${fields.join(", ")}) VALUES (${names.map((name) => (request.values[name].type === "real" ? "CAST(? AS REAL)" : "?")).join(", ")})` : "DEFAULT VALUES"}`,
      ).run(...values);
      return;
    }
