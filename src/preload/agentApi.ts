...
const agentApi: AgentDesktopApi = {
    ...
    getThreadSnapshot: (projectPath) => ipcRenderer.invoke(AGENT_IPC_CHANNELS.threadSnapshot, projectPath),
    listMessages: (threadId) => ipcRenderer.invoke(AGENT_IPC_CHANNELS.listMessages, threadId),
    createThread: (title, projectPath) => ipcRenderer.invoke(AGENT_IPC_CHANNELS.createThread, title, projectPath),
