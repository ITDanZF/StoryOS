...
const windowApi: WindowDesktopApi = {
  ...
  listFileBrowserDirectory: (request) =>
    ipcRenderer.invoke(WINDOW_IPC_CHANNELS.fileBrowserDirectory, request),
  resolveFileBrowserTarget: (request) =>
    ipcRenderer.invoke(WINDOW_IPC_CHANNELS.fileBrowserTarget, request),
  getDroppedFilePath: (file) => webUtils.getPathForFile(file),
  revealFile: (filePath) => ipcRenderer.invoke(WINDOW_IPC_CHANNELS.revealFile, filePath),
  rememberTransferLocation: (fileOrDirectoryPath) =>
    ipcRenderer.invoke(WINDOW_IPC_CHANNELS.rememberTransferLocation, fileOrDirectoryPath),
