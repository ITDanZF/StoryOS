...
你可以在书架中管理书籍、关联写作项目，按分卷和章节组织内容，在分页富文本编辑器中持续创作，也能打开阅读器预览作品；AI 可以读取当前项目与章节上下文，协助分析、续写、改写和执行更复杂的创作任务。
...
- 从书架打开阅读器，查看目录并调整阅读布局、字号和主题
- 导入或导出 StoryOS 完整备份（`.storyos-book`）、DOCX、Markdown（`.md` / `.zip`）和 TXT
- 导出 EPUB 和 PDF 阅读版；这两种格式目前不支持导入
- 完整备份包含修订历史，外部稿件格式仅交换当前正文
