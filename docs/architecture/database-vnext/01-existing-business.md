...
> 先覆盖现有功能，再设计 RAG 扩展。允许重建旧库，但本轮不执行重建，不包含 SQL 或业务代码。
...
本稿没有性能实测结论。实施后应在代表性书库和长会话上比较查询次数、I/O、P50/P95、写入放大和主进程阻塞，再确认优化收益。

## 8. 实施边界

业务结构评审通过后，再确定新 schema 版本、初始化入口、Repository 接口、IPC 兼容和验证用例。虽然允许舍弃旧数据，重建仍应是一次明确操作，与常规启动解耦。
