...
- 生成任务已从 `useAgentWorkspace.state` 迁入 `generationStore`：生成开始后选中目标章；`book_changed` 仍 reload 目录，但不卸载当前章已加载正文；修订变化时再 `loadChapter`；页级预览留在当前编辑器宿主，直到 loaded canonical 与生成正文一致。用户点走后，生成自己的 `chapter_revision_saved` 不抢回焦点。
- 阶段 2–5 尚未开始；不得把本轮删除 AI 热路径的收益当作输入、目录和搜索优化已经完成。不在本切片引入 catalog/content/pagination 四个新 store。
...
- 生成开始后选中身份跟随目标章；页级预览只进入当前编辑器宿主，不重绘目录和助手。用户可点开其他章继续写作，生成会话不中断。
...
6. 生成开始后选中目标章；预览只写入当前编辑器宿主。失败或取消时撤掉内存预览。用户点走后不在完成时抢回焦点。
